 ...
 * @author Pigalot
 */
public class TileBucketFiller extends TileMachine implements IMachine, IPowerReceptor, ILiquidContainer, IInventory, ISidedInventory {
	...
	@TileNetworkData
	public TileBucketFiller.Slot slot = new TileBucketFiller.Slot();
	...
	public void updateEntity() {
		...
		int inputLiquidId = BuildCraftAPI.getLiquidForFilledItem(getInputStack());
		...
		if (inputLiquidId == 0 && slot.quantity < BuildCraftAPI.BUCKET_VOLUME) {
			CookTime = 0;
			return;
		}
		...
		if (inputLiquidId == 0) {
			if (getInputStack().itemID == Item.bucketEmpty.shiftedIndex) {
				if (!hasPower) {
					...
					newBucketStack = BuildCraftAPI.getFilledItemForLiquid(slot.liquidId);
		...
		if (hasPower && newBucketStack != null) {
			...
			if (inputLiquidId == 0) {
				...
				fill(Orientations.Unknown, BuildCraftAPI.BUCKET_VOLUME, inputLiquidId, true);
			}
